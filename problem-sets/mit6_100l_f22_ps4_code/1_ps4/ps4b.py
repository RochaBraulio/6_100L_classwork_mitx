...
class PlaintextMessage(Message):
    def __init__(self, input_text, pad=None):
        ...
        if pad == None:
            self._pad = self.generate_pad().copy()
        else:
            self._pad = pad.copy() # shallow copy is sufficient for a list of ints
        self._chiper_text = self.apply_pad(self._pad)
    ...
    def generate_pad(self):
        ...
        pad = [random.randint(0, 110) for i in range(len(self._msg))]
        return pad
...
class EncryptedMessage(Message):
    ...
    def decrypt_message(self, pad):
        ...
        pad_copy = pad.copy()
        reverse_pad = [el*-1 for el in pad_copy] # pad to decrypt message
        decrypted_message = Message(self._msg).apply_pad(reverse_pad)
        #return Message(decrypted_message)
        return PlaintextMessage(decrypted_message, pad_copy)
