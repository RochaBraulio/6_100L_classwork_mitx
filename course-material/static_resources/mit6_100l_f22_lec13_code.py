...
def pairwise_div(Lnum, Ldenom):
    ...
    assert len(Lnum) == len(Ldenom), 'Lnum and Ldenom do not have the same length'
    assert len(Lnum) != 0 or len(Ldenom) != 0, 'Input are empty'
    try:
        Lresul = [Lnum[i]/Ldenom[i] for i in range(len(Lnum))]
        return Lresul
    except ZeroDivisionError:
