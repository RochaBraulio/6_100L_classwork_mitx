...
low = 0
high = x
ans = 0 #❗️solution does not contain this

#❗️solution does not contain if-else block below
if x < 1: 
...
while abs(ans**4 - x) > epsilon: #❗️solution is >= epsilon 
    if ans**4 - x > 0:
        high = ans
    else:
        low = ans
...
def check_within_range(user_num, user_rng_min, user_rng_max)
    if user_rng_min <= user_num <= user_rng_max:
        print(f'Number {user_num} falls within the {user_rng_min} - {user_rng_max} range')
    else:
        print(f'Number {user_num} falls outside the {user_rng_min} - {user_rng_max} range')
...
print(f'Forth root of {x} is: {ans}')
